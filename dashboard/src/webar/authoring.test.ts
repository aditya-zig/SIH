...
describe("authoring media validation", () => {
  it("rejects empty, oversized, wrong-type, and over-count selections", () => {
    ...
    expect(
      validateMediaFiles([{ name: "x.pdf", type: "application/pdf", size: 10 }]).length,
    ).toBeGreaterThan(0);
    expect(
      validateMediaFiles([{ name: "x.jpg", type: "image/jpeg", size: 60 * 1024 * 1024 }]).length,
    ).toBeGreaterThan(0);
    expect(
      validateMediaFiles(
        Array.from({ length: 11 }, (_, i) => ({ name: `${i}.jpg`, type: "image/jpeg", size: 10 })),
      ).length,
    ).toBeGreaterThan(0);
...
describe("draft store revision + approval", () => {
  it("creates AI_DRAFT with a content hash, approves the exact revision", async () => {
    ...
    const created = await store.create({
      draftId: "d1", organizationId: "o1", trainerId: "t1",
      templateId: "fire-safety-induction", templateVersion: 1, draft: shellDraft("d1"),
    });
  ...
  it("any edit bumps revision, rehashes, and clears approval", async () => {
    ...
    const edited = await store.edit("d1", (d) => ({
      ...d,
      trainingSteps: d.trainingSteps.map((s, i) =>
        i === 0 ? { ...s, instruction: "Changed instruction." } : s,
      ),
    }));
...
describe("stale generation guard", () => {
  it("rejects stale generation results that predate trainer edits", async () => {
    ...
    // Fresh pins still apply.
    const current = store.get("d1")!;
    const applied = await store.applyGeneration("d1", shellDraft("d1"), {
...
describe("draft projection", () => {
  ...
  it("projects a valid package whose entities match the draft objects", async () => {
    ...
    expect(pkg.scene.objects.map((o) => o.id).sort()).toEqual(
      created.draft.arObjects.map((o) => o.id).sort(),
    );
  });
  ...
  it("edited drafts re-project deterministically", async () => {
    ...
    const edited = await store.edit("d1", (d) => ({
      ...d,
      trainingSteps: d.trainingSteps.map((s) =>
        s.id === "identify" ? { ...s, voiceText: "Look at the panel now." } : s,
      ),
    }));
    const pkg = projectDraftToPackage(edited, { workplaceId: "w1", title: "T" });
    expect(pkg.voiceContent.lines.find((l) => l.stepId === "identify")?.text).toBe("Look at the panel now.");
  });
});
