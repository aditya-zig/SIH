...
} from "./schema/validateTrainingPackage.js";
...
describe("TrainingPackage validator (vectors 1, 2, 13)", () => {
  it("vector 1: valid Fire fixture passes", () => {
  ...
  it("vector 2: duplicate object id is rejected", () => {
    ...
    const errors = validateTrainingPackage(bad);
    expect(errors.some((e) => e.includes("Duplicate AR object id"))).toBe(true);
  });
  ...
  it("vector 2: missing referenced step is rejected", () => {
    ...
    expect(
      validateTrainingPackage(bad).some((e) => e.includes("unknown step")),
    ).toBe(true);
  });
  ...
  it("vector 2: missing mandatory Fire step is rejected", () => {
    const bad = structuredClone(fireFixturePackage);
    bad.trainingSteps = bad.trainingSteps.filter((s) => s.id !== "exit");
    bad.arObjects = bad.arObjects.filter((o) => o.relatedStep !== "exit");
    bad.scene.objects = bad.scene.objects.filter((o) => o.relatedStepId !== "exit");
    bad.evaluationRules = { ...bad.evaluationRules, requiredStepIds: ["identify", "extinguisher", "pin"] };
    expect(
      validateTrainingPackage(bad).some((e) => e.includes("mandatory step")),
    ).toBe(true);
  });
  ...
  it("vector 13: package objects map one-to-one to entities", () => {
    ...
    expect(new Set(entities.map((e) => e.objectId))).toEqual(
      new Set(fireFixturePackage.scene.objects.map((o) => o.id)),
    );
  });
