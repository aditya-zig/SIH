...
export function validateSceneDefinition(scene: unknown): string[] {
  ...
  if (typeof s.schemaVersion !== "number" || s.schemaVersion < 1)
    errors.push("scene.schemaVersion must be a positive number.");
  if (!s.sceneTemplateId || typeof s.sceneTemplateId !== "string")
    errors.push("scene.sceneTemplateId is required.");
  if (typeof s.sceneTemplateVersion !== "number" || (s.sceneTemplateVersion ?? 0) < 1)
    errors.push("scene.sceneTemplateVersion must be >= 1.");
  if (s.units !== "m") errors.push('scene.units must be "m".');
  if (!s.coordinateConvention || typeof s.coordinateConvention !== "string")
    errors.push("scene.coordinateConvention is required.");
  if (!s.trainingRoot || typeof s.trainingRoot !== "string")
    errors.push("scene.trainingRoot is required.");
  ...
  for (const [i, o] of s.objects.entries()) {
    ...
    if (!o.assetKey || typeof o.assetKey !== "string")
      errors.push(`scene.objects[${o.id ?? i}].assetKey is required.`);
    else if (!isTrustedAssetKey(o.assetKey))
      errors.push(`Untrusted assetKey rejected: ${o.assetKey}`);
    if (!isFiniteVec3(o.position))
      errors.push(`scene.objects[${o.id ?? i}].position must be finite [x,y,z].`);
    if (o.rotation !== undefined && !isFiniteVec3(o.rotation))
      errors.push(`scene.objects[${o.id ?? i}].rotation must be finite [x,y,z].`);
    if (o.scale !== undefined && !isFiniteVec3(o.scale))
      errors.push(`scene.objects[${o.id ?? i}].scale must be finite [x,y,z].`);
    if (!o.interactionType || !ALLOWED_INTERACTIONS.has(o.interactionType))
      errors.push(
        `scene.objects[${o.id ?? i}].interactionType must be one of ${[...ALLOWED_INTERACTIONS].join(", ")}.`,
      );
  }
  if (!Array.isArray(s.zones)) errors.push("scene.zones must be an array.");
  if (typeof s.lighting !== "object" || s.lighting === null)
    errors.push("scene.lighting must be an object.");
  return errors;
}
...
export function validateTrainingPackage(pkg: unknown): string[] {
  ...
  if (typeof p.version !== "number" || (p.version ?? 0) < 1)
    errors.push("version must be >= 1.");
  if (!p.templateId || typeof p.templateId !== "string")
    errors.push("templateId is required.");
  if (typeof p.templateVersion !== "number" || (p.templateVersion ?? 0) < 1)
    errors.push("templateVersion must be >= 1.");
  if (!Array.isArray(p.sourceMediaRefs)) errors.push("sourceMediaRefs must be an array.");
  if (!Array.isArray(p.trainingSteps) || p.trainingSteps.length === 0)
  ...
  if (!p.evaluationRules || typeof p.evaluationRules !== "object")
    errors.push("evaluationRules is required.");
  if (!p.voiceContent || typeof p.voiceContent !== "object")
    errors.push("voiceContent is required.");
  ...
  // Unique AR object IDs + finite positionHint + trusted assetKey.
  ...
  for (const [i, o] of (p.arObjects ?? []).entries()) {
    ...
    if (!isTrustedAssetKey(o.assetKey ?? ""))
      errors.push(`Untrusted assetKey rejected: ${o.assetKey}`);
    if (o.positionHint !== undefined && !isFiniteVec3(o.positionHint))
      errors.push(`arObjects[${o.id}].positionHint must be finite [x,y,z].`);
    if (o.relatedStep && !stepIds.has(o.relatedStep))
      errors.push(`arObjects[${o.id}] references unknown step: ${o.relatedStep}`);
  }
  // Scene cross-refs.
  if (p.scene) {
    ...
    if (Array.isArray(scene.objects)) {
      for (const o of scene.objects) {
        if (o.relatedStepId && !stepIds.has(o.relatedStepId))
          errors.push(`scene.objects[${o.id}] references unknown step: ${o.relatedStepId}`);
      }
    }
  } else {
    errors.push("scene is required.");
  }
  // Assets: reject external URLs. Trusted catalog only.
  for (const [i, a] of (p.assets ?? []).entries()) {
    if (!a?.assetKey || !isTrustedAssetKey(a.assetKey))
      errors.push(`assets[${i}] has untrusted assetKey: ${a?.assetKey}`);
    if (typeof a?.url !== "string" || /^https?:\/\//i.test(a.url))
      errors.push(`assets[${a?.assetKey ?? i}].url must be a local/catalog path, not an external URL.`);
  }
  // Evaluation rules refs must resolve.
  ...
  if (rules) {
    ...
    if (
      typeof rules.quizThresholdPercent !== "number" ||
      rules.quizThresholdPercent < 0 ||
      rules.quizThresholdPercent > 100
    )
      errors.push("evaluationRules.quizThresholdPercent must be between 0 and 100.");
  }
  // Mandatory Fire fixture steps (fixture-only gate, not safety approval).
  if (p.templateId === "fire-safety-induction") {
    for (const id of ["identify", "extinguisher", "pin", "exit"]) {
